...
OUTDIR      = r"C:\Users\liams\Documents\PhD-Project Data\Accumulation Curves"
HABITAT_COL = "habitat_name.x"   # grouping column for habitat curves
H3_RES      = 15        # H3 res (integer 0-15). 6 ~36 km2/cell. Higher = finer/noisier.
...
def main():
    ...
    t_hab = plot_overlay(df, HABITAT_COL,
    ...
    for g in df[HABITAT_COL].dropna().unique():
        safe = safe_filename(g)
        plot_single(df, HABITAT_COL, g, f"accum_habitat_{safe}.png")
    ...
    if len(trees):
        plot_overlay(trees, HABITAT_COL,
